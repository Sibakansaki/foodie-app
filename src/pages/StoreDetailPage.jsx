    ...
    <div>
      ...
      <div style={{ padding:'14px 16px',background:'#fff',borderBottom:'0.5px solid rgba(0,0,0,0.08)' }}>
        <div style={{ display:'flex',gap:8,flexWrap:'wrap',alignItems:'center',marginBottom:6 }}>
          ...
          {store.mapsUrl && (
            <a href={store.mapsUrl} target="_blank" rel="noopener noreferrer" style={{ fontSize:12,color:'#185FA5',display:'flex',alignItems:'center',gap:3 }}>
              🗺️ Google Maps
            </a>
          )}
        </div>
              ...
              <div key={set.id} style={{ marginBottom:10 }}>
                ...
                {(set.items || []).map((item, i) => (
                  <div key={i} style={{ display:'flex',alignItems:'center',gap:8,padding:'8px 13px',marginBottom:4,marginLeft:12,background:'#fff',border:'0.5px solid rgba(0,0,0,0.08)',borderLeft:'2px solid #EF9F27',borderRadius:'0 8px 8px 0' }}>
                    ...
                    {item.spicyLevel && <span style={{ fontSize:11,padding:'1px 6px',borderRadius:8,background:'#FCEBEB',color:'#A32D2D' }}>🌶 {item.spicyLevel}</span>}
...
function DishRow({ dish, onEdit, onDelete }) {
  return (
    <div style={{ display:'flex',alignItems:'center',gap:8,padding:'10px 13px',background:'#fff',border:'0.5px solid rgba(0,0,0,0.08)',borderRadius:10,marginBottom:6 }}>
      ...
      {dish.spicyLevel && <span style={{ fontSize:11,padding:'1px 6px',borderRadius:8,background:'#FCEBEB',color:'#A32D2D' }}>🌶 {dish.spicyLevel}</span>}
      ...
      {(dish.items||[]).map((item,i) => (
        <div key={i} style={{ padding:'10px',background:'#fff',borderRadius:8,marginBottom:6,borderLeft:'2px solid #EF9F27' }}>
          <div style={{ display:'flex',gap:6,marginBottom:6 }}>
            ...
            {(dish.items||[]).length > 1 && (
              <button onClick={() => removeItem(i)} style={{ background:'none',border:'0.5px solid rgba(0,0,0,0.15)',borderRadius:6,padding:'0 8px',cursor:'pointer',color:'#9a9a94' }}>×</button>
            )}
          </div>
          ...
          <div style={{ display:'flex',alignItems:'center',gap:6 }}>
            <label style={{ display:'flex',alignItems:'center',gap:4,fontSize:12,cursor:'pointer' }}>
              <input type="checkbox" checked={!!item.spicy} onChange={e => updateItem(i,'spicy',e.target.checked)}/>
              🌶 可辣
            </label>
