...
export default function StoreFormPage() {
  ...
  const [form, setForm] = useState({
    name: '', address: '', mapsUrl: '', priceRange: '', tags: [], note: '',
  })
  ...
  const toggleTag = (tid) => {
    set('tags', form.tags.includes(tid) ? form.tags.filter(t => t !== tid) : [...form.tags, tid])
  }
  ...
  const handleMapsUrl = async (url) => {
    ...
    try {
      const res = await api.expandUrl(url)
      ...
      <div style={{ padding:16 }}>
        ...
        <Field label="價位範圍">
          <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:6}}>
            {PRICES.map(p=>(
              <button key={p.value} onClick={()=>set('priceRange',form.priceRange===p.value?'':p.value)} style={{ padding:'8px 4px',borderRadius:8,border:'0.5px solid',cursor:'pointer',textAlign:'center',fontSize:12,transition:'all .15s',background:form.priceRange===p.value?'#E1F5EE':'#fff',borderColor:form.priceRange===p.value?'#1D9E75':'rgba(0,0,0,0.12)',color:form.priceRange===p.value?'#085041':'#5a5a56',fontWeight:form.priceRange===p.value?500:400 }}>
          ...
          {tags.length===0 ? <div style={{fontSize:12,color:'#9a9a94'}}>尚無標籤，請先到標籤管理新增</div> : (
            <div style={{display:'flex',flexWrap:'wrap',gap:6}}>
              {tags.map(t=>{const on=form.tags.includes(t.id);return(
                <button key={t.id} onClick={()=>toggleTag(t.id)} style={{ padding:'5px 12px',borderRadius:20,fontSize:13,border:'0.5px solid',cursor:'pointer',transition:'all .15s',background:on?'#FAECE7':'#fff',borderColor:on?'#D85A30':'rgba(0,0,0,0.12)',color:on?'#712B13':'#5a5a56',fontWeight:on?500:400 }}>
          ...
          <label style={{display:'block',cursor:'pointer'}}>
            {photoPreview ? <img src={photoPreview} alt="" style={{width:'100%',height:160,objectFit:'cover',borderRadius:8}}/> : (
              <div style={{border:'1px dashed rgba(0,0,0,0.2)',borderRadius:8,height:100,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:6,color:'#9a9a94',fontSize:13}}>
                <span style={{fontSize:28}}>📷</span>點擊上傳或拍照
              </div>
            )}
